...
import { ControlValueAccessor, NG_VALUE_ACCESSOR } from '@angular/forms'; //form control
import { MatChip, MatChipList } from '@angular/material/chips'; //needed chip components
...
export class ChipsMultiSelectComponent implements OnInit, AfterViewInit, ControlValueAccessor {
  ...
  selectChips(value: string[]){
    ...
    const chipsToSelect = this.chipList.chips.filter(
      (c) => value.includes(c.value)
    );

    chipsToSelect.forEach(
      (chip) => chip.select()
    );
  }
